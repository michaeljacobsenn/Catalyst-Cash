...
- Vault transfer rationale: every vault or savings transfer must name a specific staging target (e.g. "Stage $1,600 in Ally for Geico May 5", "Reserve $682 for Acura May 7"). Never say "transfer $X to vault" without stating which upcoming obligation it covers. If surplus is being moved to general reserves, say "General reserve — no specific obligation assigned."
- Full surplus exhaustion: if Operational Surplus > $0 after floor and obligations, deploy every dollar to a named destination in priority order (staging upcoming obligations → debt paydown → vault reserve) until $0 remains. Show a running total: "$X deployed, $0 remaining." If any surplus is left undeployed, explain why.
- Investment balance divergence: if a live-tracked investment value (Roth, Brokerage, 401k) diverges from the user-entered form value, acknowledge the discrepancy in the INVESTMENTS section. State both values and explain the delta is likely due to market movement since the form was last saved. Always use the live value for calculations.`,
    `========================
A) UX + OUTPUT RULES
========================

...
function utilizationScoreContribution(utilPct) {
    ...
    if (utilPct <= 9) return 95;
    if (utilPct <= 19) return 85;
    if (utilPct <= 29) return 75;
    if (utilPct <= 39) return 60;
    if (utilPct <= 49) return 45;
    if (utilPct <= 74) return 30;
    return 15;
}
...
export default function CreditScoreSimulator({ cards = [], financialConfig = {} }) {
    ...
    const analysis = useMemo(() => {
        ...
        const activeCards = cards.filter(c => {
            ...
            const lim = parseFloat(c.creditLimit || c.limit) || 0;
            return lim > 0 || bal > 0;
        });
        ...
        for (const c of activeCards) {
            const bal = Math.max(0, parseFloat(c.balance) || 0);
            const lim = Math.max(0, parseFloat(c.creditLimit || c.limit) || 0);
            totalBalance += bal;
            totalLimit += lim;
            if (lim > 0) {
                cardBreakdown.push({
        ...
        const scoreDelta = newContrib - currentContrib;
    ...
    const { currentUtil, newUtil, currentTier, newTier, scoreDelta, totalBalance, totalLimit, cardBreakdown, hasPaydown } = analysis;

    return (
        <Card animate delay={115} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                ...
                <span style={{
                    fontSize: 9, fontWeight: 700, color: currentTier.color,
                    padding: "3px 8px", borderRadius: 10,
                    background: `${currentTier.color}12`, border: `1px solid ${currentTier.color}25`
            ...
            <div style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, color: T.text.secondary }}>
                        Overall Utilization
                    </span>
                ...
                <div style={{
                    height: 6, borderRadius: 3, background: T.bg.elevated,
            ...
            <div style={{
                ...
                <label style={{ fontSize: 9, fontWeight: 700, color: T.text.dim, fontFamily: T.font.mono, display: "block", marginBottom: 6 }}>
                    WHAT IF I PAY DOWN...
                </label>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <input
                        ...
                        style={{
                            flex: 1, padding: "8px 10px", fontSize: 14, fontWeight: 700,
                            fontFamily: T.font.mono, background: T.bg.card,
                            border: `1px solid ${T.border.default}`, borderRadius: T.radius.sm,
                            color: T.text.primary, minHeight: "unset"
                        }}
                    />
                </div>
            </div>
                ...
                <div style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                            {scoreDelta > 0 ? <TrendingUp size={12} color={T.status.green} /> :
                             scoreDelta < 0 ? <TrendingDown size={12} color={T.status.red} /> :
                             <Minus size={12} color={T.text.dim} />}
                            <Mono size={11} weight={800} color={
                                scoreDelta > 0 ? T.status.green : scoreDelta < 0 ? T.status.red : T.text.dim
                            }>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: 8, alignItems: "center" }}>
                        <div style={{ textAlign: "center" }}>
                            ...
                            <Mono size={13} weight={800} color={currentTier.color}>{currentUtil.toFixed(1)}%</Mono>
                        </div>
                        <div style={{ fontSize: 14, color: T.text.muted }}>→</div>
                        <div style={{ textAlign: "center" }}>
                            <div style={{ fontSize: 8, color: T.text.dim, fontFamily: T.font.mono }}>PROJECTED</div>
                            <Mono size={13} weight={800} color={newTier.color}>{newUtil.toFixed(1)}%</Mono>
                        </div>
                    </div>
                    {newTier.label !== currentTier.label && (
                        <div style={{
                            marginTop: 8, padding: "4px 8px", borderRadius: 6, textAlign: "center",
                            background: `${T.status.green}10`, fontSize: 9, fontWeight: 700, color: T.status.green
                        }}>

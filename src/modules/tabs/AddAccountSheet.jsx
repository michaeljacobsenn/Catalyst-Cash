...
import { Plus, X, ArrowLeft, CreditCard, Landmark, TrendingUp, AlertTriangle, Target, Wallet, Link2, CheckCircle2 } from "lucide-react";
...
export default function AddAccountSheet({
    show, step, onClose, onSetStep,
    onAddCard, onAddBank, onAddInvestment, onAddGoal, onAddDebt, onAddAsset,
    onPlaidConnect, plaidLoading, plaidResult, plaidError, cardCatalog
}) {
    // ── Card form state ──
    const [addForm, setAddForm] = useState({ institution: "", cardChoice: "", customName: "", nickname: "", limit: "", annualFee: "" });
    ...
    const inputStyle = { padding: "12px 14px", borderRadius: T.radius.md, border: `1.5px solid ${T.border.default}`, background: T.bg.elevated, color: T.text.primary, fontSize: 13, outline: "none", transition: "all 0.2s" };

    const submitBtnStyle = (enabled, color) => ({
        padding: "14px", borderRadius: T.radius.md, border: "none",
    ...
    const SegmentedControl = ({ steps, currentStep, onStepChange, color = T.accent.primary }) => (
        <div style={{ display: "flex", borderRadius: T.radius.md, background: T.bg.card, border: `1px solid ${T.border.default}`, padding: 2, position: "relative" }}>
            <div style={{
                position: "absolute", top: 2, left: `calc(${currentStep * (100 / steps.length)}% + 2px)`,
                width: `calc(${100 / steps.length}% - 4px)`, height: "calc(100% - 4px)",
                borderRadius: T.radius.sm, background: `${color}20`,
                transition: "left 0.25s cubic-bezier(0.4, 0, 0.2, 1)", zIndex: 0
            }} />
            {steps.map((step, idx) => (
                <button key={idx} onClick={() => { haptic.selection(); onStepChange(idx); }} style={{
                    flex: 1, padding: "7px 0", border: "none", background: "transparent",
                    color: currentStep === idx ? color : T.text.dim,
                    fontSize: 10, fontWeight: currentStep === idx ? 800 : 600, cursor: "pointer",
    ...
    // Shared nav buttons
    const NavButtons = ({ step, maxStep, onPrev, onNext, onSubmit, submitLabel, canSubmit, color = T.accent.primary }) => (
        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            {step > 0 && (
                <button onClick={onPrev} style={{
                    flex: 0.6, padding: 12, borderRadius: T.radius.sm,
                    border: `1px solid ${T.border.default}`, background: "transparent", color: T.text.dim, fontSize: 11, cursor: "pointer", fontWeight: 600
                }}>← Back</button>
            )}
            <button className="hover-btn" onClick={onSubmit} disabled={!canSubmit} style={{
        ...
        <>
            ...
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "16px 20px 12px", gap: 8 }}>
                    {step && <button onClick={() => onSetStep(null)} style={{ width: 32, height: 32, borderRadius: 10, border: "none", background: T.bg.card, color: T.text.secondary, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><ArrowLeft size={16} /></button>}
                    ...
                    <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 10, border: "none", background: T.bg.card, color: T.text.secondary, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={16} /></button>
                </div>
                ...
                <div style={{ padding: "0 16px 16px" }}>
                    ...
                    {!step && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            {/* Plaid hero button */}
                            {ENABLE_PLAID && (
                                <button onClick={onPlaidConnect} disabled={plaidLoading} style={{
                                    width: "100%", padding: "14px", borderRadius: T.radius.md, border: "none",
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                                {[
                                    ...
                                    <button key={id} onClick={() => onSetStep(id)} style={{
                    ...
                    {step === "card" && (() => {
                        ...
                        return <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            ...
                            {addCardStep === 0 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>ISSUER & CARD</span>
                                ...
                                {addForm.cardChoice === "__other__" && <input value={addForm.customName} onChange={e => setAddForm(p => ({ ...p, customName: e.target.value }))} placeholder="Card name" style={inputStyle} {...focusStyle(T.accent.primary)} />}
                            </>}

                            {addCardStep === 1 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>LIMIT & FEES</span>
                                <div style={{ display: "flex", gap: 8 }}>
                                    <div style={{ flex: 1, position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.accent.primary, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={addForm.limit} onChange={e => setAddForm(p => ({ ...p, limit: e.target.value }))} placeholder="Limit" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.accent.primary)} /></div>
                                    <div style={{ flex: 1, position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.accent.primary, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={addForm.annualFee} onChange={e => setAddForm(p => ({ ...p, annualFee: e.target.value }))} placeholder="Annual Fee" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.accent.primary)} /></div>
                                </div>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>NICKNAME</span>
                                <input value={addForm.nickname} onChange={e => setAddForm(p => ({ ...p, nickname: e.target.value }))} placeholder="Nickname (optional)" style={inputStyle} {...focusStyle(T.accent.primary)} />
                            </>}

                            <NavButtons
                                step={addCardStep} maxStep={1}
                                onPrev={() => { haptic.selection(); setAddCardStep(s => s - 1); }}
                                onNext={() => { haptic.selection(); setAddCardStep(s => s + 1); }}
                                onSubmit={() => {
                                    if (!canAdd) return;
                                    onAddCard({
                                    ...
                                    setAddForm({ institution: "", cardChoice: "", customName: "", nickname: "", limit: "", annualFee: "" });
                                    setAddCardStep(0);
                                    onClose();
                                }}
                    ...
                    {step === "bank" && (() => {
                        ...
                        return <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            ...
                            {addBankStep === 0 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>INSTITUTION & TYPE</span>
                                ...
                                <div style={{ display: "flex", background: T.bg.card, borderRadius: T.radius.sm, padding: 3 }}>
                                    {["checking", "savings"].map(t => (<button key={t} onClick={() => { haptic.selection(); setAddBankForm(p => ({ ...p, accountType: t, productName: "", customName: "" })); }} style={{ flex: 1, padding: "11px", border: "none", borderRadius: T.radius.sm, background: addBankForm.accountType === t ? T.status.blue : T.bg.card, color: addBankForm.accountType === t ? "#fff" : T.text.secondary, fontSize: 13, fontWeight: 700, cursor: "pointer", textTransform: "capitalize" }}>{t}</button>))}
                                </div>
                                ...
                                {(!addBankForm.bank || bList.length === 0 || addBankForm.productName === "__other__") && <input value={addBankForm.customName} onChange={e => setAddBankForm(p => ({ ...p, customName: e.target.value }))} placeholder="Account name" style={inputStyle} {...focusStyle(T.status.blue)} />}
                            </>}

                            {addBankStep === 1 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>INTEREST RATE</span>
                                <div style={{ position: "relative" }}><input type="number" inputMode="decimal" step="0.01" value={addBankForm.apy} onChange={e => setAddBankForm(p => ({ ...p, apy: e.target.value }))} placeholder="APY (optional)" style={{ ...inputStyle, width: "100%", paddingRight: 36, boxSizing: "border-box" }} {...focusStyle(T.status.blue)} /><span style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", color: T.status.blue, fontSize: 13, fontWeight: 700 }}>%</span></div>
                            </>}

                            <NavButtons
                                step={addBankStep} maxStep={1}
                                onPrev={() => { haptic.selection(); setAddBankStep(s => s - 1); }}
                                onNext={() => { haptic.selection(); setAddBankStep(s => s + 1); }}
                                onSubmit={() => {
                                    if (!canAdd) return;
                                    onAddBank({
                                        bank: addBankForm.bank, accountType: addBankForm.accountType,
                                        name: (addBankForm.productName === "__other__" ? addBankForm.customName : addBankForm.productName).trim(),
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                                {[
                                    ...
                                    <button key={o.key} onClick={() => setInvestKey(o.key)} style={{ padding: "7px 14px", borderRadius: 99, border: `1.5px solid ${investKey === o.key ? o.color : T.border.default}`, background: investKey === o.key ? `${o.color}18` : "transparent", color: investKey === o.key ? o.color : T.text.secondary, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{o.label}</button>
                                ))}
                            </div>
                            <SearchableSelect value={investSym} onChange={setInvestSym} placeholder={investKey === "crypto" ? "Search crypto…" : "Search ticker…"} options={[...getTickerOptions(investKey).map(c => ({ value: c.symbol, label: `${c.symbol.replace("-USD", "")} — ${c.name}` })), { value: "__custom__", label: "Custom ticker…" }]} />
                            {investSym === "__custom__" && <input placeholder="Enter ticker symbol" onChange={e => setInvestSym(e.target.value.toUpperCase())} style={inputStyle} {...focusStyle(T.accent.emerald)} />}
                            <input type="number" inputMode="decimal" value={investShares} onChange={e => setInvestShares(e.target.value)} placeholder={investKey === "crypto" ? "Amount" : "Shares"} style={inputStyle} {...focusStyle(T.accent.emerald)} />
                            <button className="hover-btn" onClick={() => {
                                ...
                                setInvestKey("brokerage"); setInvestSym(""); setInvestShares("");
                                onClose();
                            }} disabled={!(investSym?.trim() && parseFloat(investShares || 0) > 0)} style={submitBtnStyle(!!(investSym?.trim() && parseFloat(investShares || 0) > 0), `linear-gradient(135deg, ${T.accent.emerald}, #0ca678)`)}>Add Holding</button>
                        </div>
                    )}
                    ...
                    {step === "goal" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            <input value={goalForm.name} onChange={e => setGoalForm(p => ({ ...p, name: e.target.value }))} placeholder="Goal name (e.g. Emergency Fund)" style={inputStyle} {...focusStyle(T.accent.primary)} />
                            <div style={{ display: "flex", gap: 8 }}>
                                <div style={{ flex: 1, position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.accent.primary, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={goalForm.targetAmount} onChange={e => setGoalForm(p => ({ ...p, targetAmount: e.target.value }))} placeholder="Target" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.accent.primary)} /></div>
                                <div style={{ flex: 1, position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.accent.primary, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={goalForm.currentAmount} onChange={e => setGoalForm(p => ({ ...p, currentAmount: e.target.value }))} placeholder="Current" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.accent.primary)} /></div>
                            </div>
                            <input type="date" value={goalForm.targetDate} onChange={e => setGoalForm(p => ({ ...p, targetDate: e.target.value }))} style={{ ...inputStyle, fontFamily: T.font.sans, fontWeight: 700 }} {...focusStyle(T.accent.primary)} />
                            <button className="hover-btn" onClick={() => {
                                if (!goalForm.name.trim()) return;
                                haptic.success();
                                onAddGoal({ name: goalForm.name.trim(), targetAmount: parseFloat(goalForm.targetAmount) || 0, currentAmount: parseFloat(goalForm.currentAmount) || 0, targetDate: goalForm.targetDate });
                                setGoalForm({ name: "", targetAmount: "", currentAmount: "", targetDate: "" });
                                onClose();
                            }} disabled={!goalForm.name.trim()} style={submitBtnStyle(!!goalForm.name.trim(), T.accent.gradient)}>Add Goal</button>
                        </div>
                    )}
                    ...
                    {step === "debt" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            ...
                            {addDebtStep === 0 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>NAME & TYPE</span>
                                <input value={debtForm.name} onChange={e => setDebtForm(p => ({ ...p, name: e.target.value }))} placeholder="Debt name (e.g. Auto Loan)" style={inputStyle} {...focusStyle(T.status.amber)} />
                                <div style={{ display: "flex", gap: 8 }}>
                                    {[
                                        { key: "auto", label: "Auto" }, { key: "student", label: "Student" },
                                        { key: "mortgage", label: "Mortgage" }, { key: "personal", label: "Personal" }, { key: "medical", label: "Medical" }
                                    ].map(t => (
                                        <button key={t.key} onClick={() => setDebtForm(p => ({ ...p, type: t.key }))} style={{ flex: 1, padding: "8px 0", borderRadius: T.radius.sm, border: `1px solid ${debtForm.type === t.key ? T.status.amber : T.border.default}`, background: debtForm.type === t.key ? `${T.status.amber}18` : T.bg.elevated, color: debtForm.type === t.key ? T.status.amber : T.text.secondary, fontSize: 11, fontWeight: 700, cursor: "pointer", boxSizing: "border-box" }}>{t.label}</button>
                                    ))}
                                </div>
                            </>}

                            {addDebtStep === 1 && <>
                                <span style={{ fontSize: 9, color: T.text.dim, fontFamily: T.font.mono, fontWeight: 700, letterSpacing: 0.5 }}>BALANCE & RATES</span>
                                <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.status.amber, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={debtForm.balance} onChange={e => setDebtForm(p => ({ ...p, balance: e.target.value }))} placeholder="Current Balance" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.status.amber)} /></div>
                                <div style={{ display: "flex", gap: 8 }}>
                                    <div style={{ flex: 1, position: "relative" }}><input type="number" inputMode="decimal" value={debtForm.apr} onChange={e => setDebtForm(p => ({ ...p, apr: e.target.value }))} placeholder="Interest APR" style={{ ...inputStyle, width: "100%", paddingRight: 28, boxSizing: "border-box" }} {...focusStyle(T.status.amber)} /><span style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", color: T.text.secondary, fontSize: 13, fontWeight: 700 }}>%</span></div>
                                    <div style={{ flex: 1, position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.text.secondary, fontSize: 12, fontWeight: 700 }}>Min $</span><input type="number" inputMode="decimal" value={debtForm.minPayment} onChange={e => setDebtForm(p => ({ ...p, minPayment: e.target.value }))} placeholder="0.00" style={{ ...inputStyle, width: "100%", paddingLeft: 48, boxSizing: "border-box" }} {...focusStyle(T.status.amber)} /></div>
                                </div>
                            </>}

                            <NavButtons
                                step={addDebtStep} maxStep={1}
                                onPrev={() => { haptic.selection(); setAddDebtStep(s => s - 1); }}
                                onNext={() => { haptic.selection(); setAddDebtStep(s => s + 1); }}
                                onSubmit={() => {
                                    if (!debtForm.name.trim()) return;
                                    haptic.success();
                                    onAddDebt({ name: debtForm.name.trim(), type: debtForm.type, balance: parseFloat(debtForm.balance) || 0, apr: parseFloat(debtForm.apr) || 0, minPayment: parseFloat(debtForm.minPayment) || 0 });
                    ...
                    {step === "asset" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            <input value={assetForm.name} onChange={e => setAssetForm(p => ({ ...p, name: e.target.value }))} placeholder="Asset name (e.g. Vehicle, Property)" style={inputStyle} {...focusStyle(T.accent.copper)} />
                            <div style={{ position: "relative" }}><span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: T.accent.copper, fontSize: 14, fontWeight: 700 }}>$</span><input type="number" inputMode="decimal" value={assetForm.value} onChange={e => setAssetForm(p => ({ ...p, value: e.target.value }))} placeholder="Estimated value" style={{ ...inputStyle, width: "100%", paddingLeft: 28, boxSizing: "border-box" }} {...focusStyle(T.accent.copper)} /></div>
                            <button onClick={() => setAssetForm(p => ({ ...p, liquid: !p.liquid }))} style={{ padding: "12px 16px", borderRadius: T.radius.md, border: `1.5px solid ${assetForm.liquid ? T.accent.emerald : T.border.default}`, background: assetForm.liquid ? `${T.accent.emerald}12` : "transparent", color: assetForm.liquid ? T.accent.emerald : T.text.secondary, fontSize: 13, fontWeight: 700, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 8 }}>{assetForm.liquid ? "💧" : "🔒"} {assetForm.liquid ? "Liquid (can access quickly)" : "Illiquid (locked up / long-term)"}</button>
                            <button className="hover-btn" onClick={() => {
                                if (!assetForm.name.trim()) return;
                                haptic.success();
                                onAddAsset({ name: assetForm.name.trim(), value: parseFloat(assetForm.value) || 0, liquid: assetForm.liquid });
                                setAssetForm({ name: "", value: "", liquid: false });
                                onClose();
                            }} disabled={!assetForm.name.trim()} style={submitBtnStyle(!!assetForm.name.trim(), `linear-gradient(135deg, ${T.accent.copper}, #e67e22)`)}>Add Asset</button>
                        </div>
                    )}
                </div>
            </div>

...
export default function DebtFreedomCard({ cards = [], freedomStats }) {
    ...
    return (
        <Card animate delay={100} style={{
            padding: "20px 18px",
            background: `linear-gradient(160deg, ${T.bg.card}, ${T.status.green}06)`,
            borderColor: `${T.status.green}18`,
            borderLeft: `3px solid ${T.status.green}`,
            overflow: "hidden", position: "relative"
        }}>
            {/* Ambient glow */}
            <div style={{
                position: "absolute", top: -40, right: -40, width: 120, height: 120,
                borderRadius: "50%", background: `${T.status.green}06`,
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                <div style={{
                    ...
                    background: T.status.greenDim, display: "flex",
                    alignItems: "center", justifyContent: "center"
                }}>
                    <Target size={14} color={T.status.green} />
                </div>
                <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 14 }}>
                <div>
                    <div style={{ fontSize: 10, fontWeight: 700, color: T.text.dim, fontFamily: T.font.mono, marginBottom: 4, letterSpacing: "0.08em" }}>
                        TOTAL DEBT
                    </div>
                    <Mono size={26} weight={800} color={T.text.primary}>{fmt(totalDebt)}</Mono>
                </div>
                {hasProjection && (
                    <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: T.status.green, fontFamily: T.font.mono, marginBottom: 4, letterSpacing: "0.08em" }}>
                            DEBT-FREE BY
                        </div>
                        <Mono size={20} weight={800} color={T.status.green}>{freeDateStr}</Mono>
                    </div>
                )}
            </div>
            ...
            {hasProjection && weeksToFree && (
                <div style={{ marginBottom: 14 }}>
                    <div style={{
                        ...
                        background: `${T.status.green}15`, overflow: "hidden"
                    }}>
                        <div style={{
                            height: "100%", borderRadius: 3,
                            background: `linear-gradient(90deg, ${T.status.green}, ${T.accent.emerald || T.status.green})`,
                            ...
                            boxShadow: `0 0 8px ${T.status.green}40`
                        }} />
                    </div>
                </div>
            )}
            ...
            {hasProjection && (
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                    ...
                    {interestSaved > 100 && (
                        <div style={{
                            padding: "6px 10px", borderRadius: T.radius.sm,
                            background: T.status.greenDim, border: `1px solid ${T.status.green}20`,
                            display: "flex", alignItems: "center", gap: 5
                        }}>
                            <span style={{ fontSize: 11 }}>💰</span>
            ...
            {/* Motivational message */}
            <p style={{
                fontSize: 11, color: T.text.dim, margin: 0,
